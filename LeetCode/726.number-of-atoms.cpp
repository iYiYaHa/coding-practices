...
private:
    map<string, int> parseFormula(string formula)
    {
        ...
        while (end < formula.size())
        {
            ...
            { // subformula
                int left_cnt = 1;
                while (end < formula.size() && left_cnt != 0)
                {
                    if (formula[end] == '(')
                        ++left_cnt;
                    else if (formula[end] == ')')
                ...
                subRes = parseFormula(formula.substr(beg + 1, end - beg - 2));
            }
            else
            {
                while (end < formula.size() && !isdigit(formula[end]) && (beg == end || !isupper(formula[end])) && formula[end] != '(')
                {
                    ++end;
                }
...
public:
    ...
    {
        map<string, int> res = parseFormula(formula);
        string resStr;
