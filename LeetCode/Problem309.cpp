...
//		dp[0][0] = -prices[0];
//		dp[1][0] = std::max(dp[0][0], -prices[1]);
//		dp[1][1] = std::max(0, prices[1] - prices[0]);
//		for (int i = 2; i < prices.size(); ++i) {
//			dp[i][0] = std::max(dp[i - 1][0], dp[i - 2][1] - prices[i]);  //hold
//			dp[i][1] = std::max(dp[i - 1][1], dp[i - 1][0] + prices[i]); //unhold
//		}
...
//			dp[i][2] = dp[i - 1][0] + prices[i];
//		}
//		return std::max(dp[n - 1][2], dp[n - 1][1]);
//	}
//};
//int main() {
...
//	std::vector<int> testAns = { 3 };
//	for (int i = 0; i < testCase.size(); ++i) {
//		bool res = SolutionA().maxProfit(testCase[i]) == testAns[i];
//		if (res) {
...
//	return 0;
//}
>>>>>>> 99022fe89f096c2f05a28f3fd38c5e83a5c40972
