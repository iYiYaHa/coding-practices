...
public:
	...
	{
		std::vector<int> table(366, 0);
		for (int day : days)
			table[day] = std::numeric_limits<int>::max();
		for (int i = 1; i <= 365; ++i)
		{
			if (table[i] == 0)
				table[i] = table[i - 1];
			else
			{
				...
				table[i] = std::min(table[i], table[std::max(0, i - 7)] + costs[1]);
				table[i] = std::min(table[i], table[std::max(0, i - 30)] + costs[2]);
			}

		}
		return table[days.back()];
	}
};
