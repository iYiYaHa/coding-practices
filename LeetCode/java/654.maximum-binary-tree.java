...
class Solution {
    ...
    private TreeNode constructMaximumBinaryTree(int[] nums, int beg, int end) {
        ...
        for (int i = beg; i <= end; ++i) {
            maxInd = nums[i] > nums[maxInd] ? i : maxInd;
        }
        TreeNode left = constructMaximumBinaryTree(nums, beg, maxInd - 1);
        TreeNode right = constructMaximumBinaryTree(nums, maxInd + 1, end);
        return new TreeNode(nums[maxInd], left, right);
    }
}
// @lc code=end
