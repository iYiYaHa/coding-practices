...
// fromi != toi 
// 
// Related Topics Depth-First Search Graph Eulerian Circuit 👍 3448 👎 1507
...
class Solution {
    ...
    public List<String> findItinerary(List<List<String>> tickets) {
        ...
        for (List<String> ticket : tickets) {
            ...
            graph.computeIfAbsent(departure, i -> new PriorityQueue<>()).add(arrival);
        }

        // Find the valid itinerary
        String source = "JFK";
        LinkedList<String> path = new LinkedList<>();
        dfs(graph, source, tickets.size(), path);
        return path;
    }

    private void dfs(Map<String, PriorityQueue<String>> graph, String source, int totalTickets, LinkedList<String> path) {
        PriorityQueue<String> pq = graph.getOrDefault(source, new PriorityQueue<>());
        while (!pq.isEmpty()) {
            dfs(graph, pq.poll(), totalTickets, path);
        }
