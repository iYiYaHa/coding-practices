...
// endj < startj+1 
// 
// Related Topics Array Two Pointers 👍 3088 👎 70
...
class Solution {
    public int[][] intervalIntersection(int[][] firstList, int[][] secondList) {
        ...
        for (int indFirst = 0, indSecond = 0; indFirst < firstList.length && indSecond < secondList.length; ) {
            int[] intervalFirst = firstList[indFirst];
            int[] intervalSecond = secondList[indSecond];
            if (isOverlap(intervalFirst, intervalSecond)) {
                res.add(new int[]{Math.max(intervalFirst[0], intervalSecond[0]), Math.min(intervalFirst[1], intervalSecond[1])});
            }
            if (intervalFirst[1] > intervalSecond[1]) {
                ++indSecond;
            } else {
                ++indFirst;
            }
