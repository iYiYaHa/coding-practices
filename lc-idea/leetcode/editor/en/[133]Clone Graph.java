...
//node. 
// 
// Related Topics Hash Table Depth-First Search Breadth-First Search Graph 👍 39
//68 👎 1961
...
class Solution {
    public Node cloneGraph(Node node) {
        ...
        return cloneNode(node, clonedNodeMap);
    }

    private Node cloneNode(Node node, Map<Node, Node> clonedNodeMap){
        if(node == null){
            return null;
        }
        ...
        Node cloned = new Node(node.val, clonedNeighbors);
        clonedNodeMap.put(node, cloned);
        for(Node neighbor : node.neighbors){
            clonedNeighbors.add(cloneNode(neighbor, clonedNodeMap));
        }
        return cloned;
    }
}
